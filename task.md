...
- [ ] **🎯 PENDIENTE — Validar E2E (Néctor + Miguel) con el código actual** *(lo corre el usuario)* — Worker apagado y jobs limpiados a pedido. Para correr: `bash scripts/sistema.sh start` y luego `npx ts-node -r dotenv/config casos/nector_ruiz/enqueue.ts` + `casos/miguel_lugo/enqueue.ts` (Centinela cacheado v15 → sin gasto de LLM). **Confirmar:**
  - **Miguel** ahora NO falla en el mapeador (fix `document_filename` null) y declara los **3 CCAF Los Andes** (alias) + **BCI $615** + **sin duplicado de Tenpo** (paridad con la abogada).
  - **Néctor** 12/12 sin regresión.
  - **Alertas claras** en `automation_alerts` (panel `/automatizacion`): mensaje en lenguaje llano, nombres limpios, solo los acreedores accionables (sin el ruido del remanente $13 ni de los 260→261).
  - Estado conocido de los jobs previos: Néctor v15 `success` (con alertas viejas/densas), Miguel v15 **`failed`** (mapeador null — ya corregido). Verificar contra los screenshots de la abogada en `casos/<cliente>/screenshots/`.
  - **Commitear todo el arco P0.f tras validar** (rama `pm/casos-comparacion-abogada`; incluye `cert_line_items.ts` nuevo).
