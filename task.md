...
- [ ] **Productizar el proyector** — sacar `tools/mantenimiento/project_case.ts` a `src/` (módulo del worker) y que el worker proyecte on-demand al tomar un job con `airtable_id`/`rut`.
...
### Pendientes Paso 5
- [x] **Alertas del agente de ingresos → dashboard (2026-07-09)**: `gatherStep5Input` ahora consolida
  `ingresosOutput.alerts` + `claudeReadIssues` en una `automation_alert` (step 5, `needs_review`) —antes se
  descartaban en el `return` y solo quedaban en `agent_runs`/logs (G2: nunca en silencio; consistencia con Paso 3).
- [x] **Paso 5 individual habilitado (2026-07-09)**: el guard de `worker.ts` acepta `step===5` (existía la rama
  pero era inalcanzable). Producción sigue usando el flujo completo `step===0`; `--step=5` queda para PRUEBAS.
...
- [x] **Per-doc ENCENDIDO por default en producción (2026-07-09)**: `sentinel.ts` usa
  `CENTINELA_PER_DOC !== 'false'` → el Paso 3 lee **un documento a la vez** por default (fix de L14); la
  mega-llamada queda como fallback explícito (`CENTINELA_PER_DOC=false`).
- [ ] **⚠️ Gate antes del primer `DRY_RUN=false`**: validación EN VIVO fresca del per-doc (`scorecard.ts 3`
  sobre un caso con verdad-terreno, ej. Yasmín/Cristian) — confirmar lectura estable con cuota de API.
