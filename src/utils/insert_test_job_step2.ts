...
async function insertTestJobStep2() {
  ...
  console.log('→ Insertando nuevo job del Paso 2 pendiente en la cola "automation_jobs" (con Dry Run activado)...');
  ...
  if (jobError) {
    ...
    console.log('🎉 Job de Paso 2 insertado correctamente en Supabase!');
    console.log('Detalles del Job:');
    console.table(jobs);
    console.log('\n👉 Para iniciar el worker y procesar este job ejecuta:');
    console.log('   npx ts-node src/worker.ts');
  }
}
