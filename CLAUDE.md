...
Pipeline general (no hardcodeado): `gatherStep5Input` (worker) reúne los docs de ingreso de `client_documents` por keyword de filename → `runIngresosAgent` (Claude **lee nativo, UNA LLAMADA POR DOCUMENTO**) → `income_extractor.ts` (capa determinista BULLETPROOF) → `fillStep5` (Playwright, tras el Paso 4). Reglas clave (detalle en `lecciones/paso5-ingresos.md` L1–L14, **reglas oficiales Superir** verificadas ahí):
- **Monto = "Líquido a pagar"** y sinónimos ("Líquido a Cobrar/Recibir", "Rem. Neta", "Monto Líquido") — NUNCA "Alcance Líquido"/"Imponible" (sobre-declara). El ingreso real = líquido + **descuentos voluntarios** redirigibles (préstamo empleador/CCAF/caja, APV); los **legales** (AFP/salud/cesantía/impuesto) y los **anticipos/sindicato/seguros** NO se suman (caen a "ambiguo" → se alertan). Clasificación por keyword con **límite de palabra izquierdo** (evita falsos positivos tipo "sis" en "asistencia").
...
- **Promedio mensualizado (L3):** permanentes (sueldo/pensión/arriendo) = últimos **3 meses** (ordenados por fecha, dedup de duplicados); honorarios = **12 meses** (divisor fijo, declara bruto + alerta). **Licencia médica:** subsidio fragmentado → dedup + reconstrucción por mes calendario + mes más completo. Periodicidad declarada **siempre Mensual** (value 4) salvo única vez. ⚠️ El portal NO dicta cómo calcular el monto (es criterio del abogado); fija los **documentos** de respaldo.
...
- **Pruebas (no producción, en `casos/paso5_pruebas/`):** `npm test` corre la suite determinista (91 unit incl. fuzz 1000× + 5 casos reales). Revisión + criterios "bulletproof" en `REVISION_Y_PLAN.md`. Fase 2 (lectura nativa real) en `run_native.ts` (pendiente API).
