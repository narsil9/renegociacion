...
- **Pruebas (no producción, en `casos/paso5_pruebas/`):** `npm test` corre la suite determinista (**106 unit** incl. fuzz 1000× + B10 de los fixes nuevos + **5 casos** lote `casos-paso5` + **11 casos** lote `renegociacion_docs`). Revisión + criterios "bulletproof" en `REVISION_Y_PLAN.md`. Fase 2 (lectura nativa real) en `run_native.ts` (pendiente API). Decisiones de criterio del abogado en `PREGUNTAS_ABOGADO_PASO5.md`.
